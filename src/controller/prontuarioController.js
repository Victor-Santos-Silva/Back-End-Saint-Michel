...
const prontuarioController = {
    create: async (req, res) => {
        try {
            const { usuario_id, problemaRelatado, recomendacaoMedico } = req.body;

            const novoCadastro = await prontuarioService.create({
                usuario_id,
                problemaRelatado,
                recomendacaoMedico
            });
    ...
    getAll: async (req, res) => {
        try {
            const prontuario = await prontuarioService.getAll();
    ...
    getOne: async (req, res) => {
        try {
            const prontuario = await prontuarioService.getById(req.params.id);
            if (!prontuario) {
    ...
    delete: async (req, res) => {
        try {
            const { id } = req.params;
            const deletado = await prontuarioService.delete(id);
